...
- **Banned vocabulary** - the fleet lint's WRITING ban list applies in full, plus the book contract's additions; against every `content/**/*.md` page, `kanon lint --writing` (version pinned once by `REQUIRED_KANON_VERSION` in `bin/check-site.sh`) runs as a stage of `python3 bin/site.py gate` and fails the gate on any violation wherever kanon is installed. kanon has no public release binary yet (#93), so `.github/workflows/deploy.yml` cannot install it; on that entrypoint the stage reports the check as UNVERIFIED instead of skipping it silently or failing the gate outright. Not restated here (the vault copy at theke `site-design/VOICE-WEB.md` carries the verbatim list). Quoting a banned word to ban it still ships the word.
...
- **The copula dodge** — inflated substitutes for a direct statement of identity, possession or action. Plain is/has/does.
...
- Stock filler openers are banned regardless of punctuation — a legal dash does not launder the phrase.

**Tiering** — the bans on generic filler are hygiene, true of any terse engineer. The signature is elsewhere: dash-glued acknowledgments, inline chaining, both-ways hedging, label-colons, genuine questions. Prose that only clears hygiene has not found the voice.
