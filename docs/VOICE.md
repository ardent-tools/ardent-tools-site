...
- **Banned vocabulary** - the fleet lint's WRITING ban list applies in full, plus the book contract's additions; enforced by `kanon lint --writing` (kanon 0.11.0, pinned in `bin/check-site.sh`) against every `content/**/*.md` page as a stage of `python3 bin/site.py gate`, not restated here (the vault copy at theke `site-design/VOICE-WEB.md` carries the verbatim list). Quoting a banned word to ban it still ships the word.
