...
- **Strict CSP**: enforced via `_headers` and mirrored in `bin/header_contract.py` and the error-boundary Function. No inline scripts/styles anywhere. System pages that publish a cast need the vendored WebAssembly player, so `script-src` carries `'wasm-unsafe-eval'` under `/systems/*` only — every other path, including the systems catalog itself when it holds no cast, keeps plain `'self'`. `bin/validate-site.py` fails if cast, player, and CSP state diverge (drop every cast and the exception must drop with it).
...
| `templates/journal-entry.html` | Shadow | Wraps `page.content` in `.prose` — the reading-measure cap every other content template already applies. The theme has since dropped its own hardcoded `/lexicon/` link, so that half of the delta is closed and only the `.prose` wrap remains. |
...
**No `partials/assert.html` import in the shadows here.** The theme ships that macro and its own templates use it to fail closed on missing frontmatter fields. The shadows in this repo predate it and do not import it, so they render without those assertions; templates that are NOT shadowed (`section.html`, `sizing-guide.html`) do get them. Adopting the asserts in a shadow means also carrying the frontmatter fields they demand — `extra.audience`, `extra.words_source`, `extra.price_source` — on every page that renders through it.
