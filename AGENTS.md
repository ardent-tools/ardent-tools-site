...
| `templates/journal-entry.html` | Shadow | Drops the substrate's hardcoded `/lexicon/` link on the components line - sibling-site furniture this site does not have (upstream: typikon#27 thread). Also wraps `page.content` in `.prose` — the reading-measure cap every other content template already applies. |
