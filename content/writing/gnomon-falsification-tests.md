...
A narrower, differently-testable claim survived both kills. Instead of many units voting on the same answer, one integrator evaluates and synthesizes across units that explore differently-scoped regions of a decomposed problem - the pattern behind Monte Carlo tree search, ant-colony methods, and a jury whose value is spread of perspective rather than any one juror's brilliance. That version doesn't need routing to capture a teacher's trained identity.
...
Gnomon is still in specification. No training run has executed. The reframed thesis has its own gate ahead of it - comparing structurally-guided specialization against a standard fine-tune on inter-unit correlation, at matched quality, before any of the reframed architecture gets built.
