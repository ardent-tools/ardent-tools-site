...
| Recording contract | [asciinema-player](https://github.com/asciinema/asciinema-player) v3.17.0 is vendored and self-hosted. Its CSS and JavaScript load only on a system page that publishes a real `cast` - today that is kanon; every other system requests no player asset. Each cast ships with its reproduction recipe and a full text transcript |
...
The authored site has no analytics, cookies, contact form, newsletter signup, remote font or script dependency, cadence promise on `/writing/`, inflated maturity label, or fabricated terminal output. Cloudflare Pages remains the delivery boundary. One cast is published so far - kanon's; the remaining recording plans appear only as plans, and no page requests a player asset without a real cast behind it.
