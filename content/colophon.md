...
| Software bill of materials | [sbom.cdx.json](/sbom.cdx.json) is a CycloneDX 1.6 JSON receipt, derived at build time from the deploy workflow's pins, three declared npm toolchain packages (`@playwright/test`, `pa11y-ci`, `wrangler`), the Python requirements, and the vendored player's bytes. It names those three directly rather than walking the npm lockfile - no transitive npm dependency or edge is included. Each component carries its purl, license, and an `ardent:tier` of `runtime` or `build`. Only asciinema-player is `runtime` - everything else builds the site and never reaches a visitor's browser. Deterministic output, no timestamp or serial number, so the gate rejects a stale copy byte-for-byte |
