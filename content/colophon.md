...
| Cache identity | Non-redirect static site responses are configured and verified `no-store, no-transform`. Cloudflare Pages evaluates `_redirects` before `_headers`; every supported redirect declaration is validated locally and probed in production for its exact permanent status and same-origin destination, without claiming cache headers on the redirect response. Non-canonical public resource URLs carry one content digest and the release asset epoch, and a retained-tree manifest checks every non-HTML artifact against its exact live bytes |
