...
| CSP - script-src | `'self'` everywhere. Only `/systems/*` - the section a system page's cast can render under - adds `'wasm-unsafe-eval'`, the single narrow exception the vendored WebAssembly player needs, never `'unsafe-inline'` or `'unsafe-eval'`. It exists only because a real cast is published somewhere in that section. The gate drops it the moment the last cast does |
