...
| Software bill of materials | [sbom.cdx.json](/sbom.cdx.json) is a CycloneDX 1.6 JSON receipt, derived at build time from the deploy workflow's pins, the npm lockfile, the Python requirements, and the vendored player's bytes. Each component carries its purl, license, and an `ardent:tier` of `runtime` or `build`. Only asciinema-player is `runtime` - everything else builds the site and never reaches a visitor's browser. Deterministic output, no timestamp or serial number, so the gate rejects a stale copy byte-for-byte |
