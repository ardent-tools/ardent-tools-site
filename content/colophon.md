...
| CSP - script-src | `'self'` plus `'wasm-unsafe-eval'` - the single narrow exception the vendored WebAssembly player needs, never `'unsafe-inline'` or `'unsafe-eval'`. It exists only because a real cast is published. The gate drops it the moment the last cast does |
