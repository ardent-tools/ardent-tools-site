...
| Supply chain | The build toolchain is pinned by digest - Zola, Typst, and lychee by SHA-256, the GitHub Actions by commit. The npm gate and deploy tools (Playwright, pa11y, wrangler) install from a committed lockfile with `npm ci`. The one Python dependency is hash-locked and installed with `--require-hashes` under a pinned interpreter. None of this build-toolchain pinning reaches a visitor's browser - the only third-party JavaScript the built site ships is the vendored asciinema-player above, and it loads only where a real cast is published |
| Software bill of materials | [sbom.cdx.json](/sbom.cdx.json) is a CycloneDX 1.6 JSON receipt, derived at build time from the deploy workflow's pins, the npm toolchain packages (all 255 from the lockfile), the Python requirements, and the vendored player's bytes. It walks `package-lock.json` in full and emits the dependency edges npm resolves between them, so a transitive package can be traced to the direct dependency that reaches it. Each component carries its purl, license where the lockfile records one, its Subresource-Integrity digest, and an `ardent:tier` - `runtime`, `build`, `test`, or `deploy`. A package reachable from more than one root carries more than one tier, because it genuinely runs in each. Only asciinema-player is `runtime` - everything else builds, tests, or deploys the site and never reaches a visitor's browser. Deterministic output, no timestamp or serial number, so the gate rejects a stale copy byte-for-byte |
...
| CSP - script-src | `'self'` everywhere, including the systems catalog. Only `/systems/*/` - an individual system page, the one place a cast can render - adds `'wasm-unsafe-eval'`, the single narrow exception the vendored WebAssembly player needs, never `'unsafe-inline'` or `'unsafe-eval'`. It exists only because that page publishes a real cast. The gate drops it the moment the last cast does |
...
The authored site has no analytics, cookies, contact form, newsletter signup, remote font or script dependency, or cadence promise on `/writing/`. Cloudflare Pages remains the delivery boundary. The [evidence register](/evidence/) lists every published cast. The systems without one show their recording plans only as plans, and no page requests a player asset without a real cast behind it.
