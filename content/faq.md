...
a = "Yes, for live retrieval on a visitor's behalf. [llms.txt](/llms.txt) indexes the structured surfaces - the [machine-readable systems catalog](/systems.json), the [career-claims receipt](/career-claims.json), and this repository's own AGENTS.md. The first two are generated, and the gate fails if either disagrees with what the human pages render. The [AI use policy](https://github.com/ardent-tools/ardent-tools-site/blob/main/AI-USE-POLICY.md) draws the line at bulk collection into a training corpus, which stays prohibited without written consent."
