...
Not wired in this scaffold. The fleet convention (matching typikon and the sibling `ardent-site` repo) is `origin` = the forkwright forge, `github` = a push-mirror; wire both before the first real push, and run `kanon forge init forkwright/ardent-tools` to create the bare forge repo first.
